        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            {errors.data?.education && (
              <p className="text-error text-sm">{errors.data?.education.message}</p>
            )}
            <div className="mt-3 flex flex-wrap gap-2 max-h-24 overflow-y-auto pr-1">
              {selectedEducation.map((education: string) => (
                <span
                  key={education}
                  className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium bg-indigo-100 text-indigo-700 badge badge-primary"
                >
                  {education}
                  <button
                    type="button"
